...
var RadarChart = {
  draw: function(id, data, options) {
    ...
    var config = {
      ...
      maxValues: {personnel: 50, criticality: 40, size: 310, culture: 100, dynamism: 75},
      maxValue: 0,
      radians: 2 * Math.PI,
    ...
    function updateConfig() {
      ...
      for(axis in vis.allAxis){
          axis = (vis.allAxis[axis])
          config.maxValues[axis] = Math.max(config.maxValues[axis],
          d3.max(data, function(d){
              return d3.max(d.axes, function(o) { 
                  value = 0;
                  if (o.axis === axis) value = o.value;
                  return value+10;})
            })
          );
      };
        ...
        group.axes.forEach(function(d, i) {
            if(d.axis === 'size'){
                 d.coordinates = { // [x, y] coordinates
                    x: config.w / 2 * (1 - (parseFloat(Math.max(Math.log(d.value), 0)) / Math.log(config.maxValues[d.axis])) * Math.sin(i * config.radians / vis.totalAxes)),
                    y: config.h / 2 * (1 - (parseFloat(Math.max(Math.log(d.value), 0)) / Math.log(config.maxValues[d.axis])) * Math.cos(i * config.radians / vis.totalAxes))
                };
            } else if (d.axis === 'culture' || d.axis === 'dynamism'){
                d.coordinates = { // [x, y] coordinates
                    x: config.w / 2 * (1 - ((config.maxValues[d.axis]- parseFloat(Math.max(d.value, 0))) / config.maxValues[d.axis]) * Math.sin(i * config.radians / vis.totalAxes)),
                    y: config.h / 2 * (1 - ((config.maxValues[d.axis]- parseFloat(Math.max(d.value, 0))) / config.maxValues[d.axis]) * Math.cos(i * config.radians / vis.totalAxes))
                };
            } else {
                d.coordinates = { // [x, y] coordinates
                    x: config.w / 2 * (1 - (parseFloat(Math.max(d.value, 0)) / config.maxValues[d.axis]) * Math.sin(i * config.radians / vis.totalAxes)),
                    y: config.h / 2 * (1 - (parseFloat(Math.max(d.value, 0)) / config.maxValues[d.axis]) * Math.cos(i * config.radians / vis.totalAxes))
                };
            }
        });
      });
